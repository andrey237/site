...
function fabulous_fluid_flush_transients(){
	...
	delete_transient( 'fabulous_fluid_favicon' );

	delete_transient( 'fabulous_fluid_webclip' );
...
if ( ! function_exists( 'fabulous_fluid_continue_reading' ) ) :
	...
	function fabulous_fluid_continue_reading() {
		...
		return ' <span class="readmore"><a href="'. esc_url( get_permalink() ) . '">' .  sprintf( wp_kses_post( __( '%s', 'fabulous-fluid' ) ) , $more_tag_text ) . '</a></span>';
	}
